...
if __name__ == '__main__':
    # sql_str = f'SELECT MAX(m_order) FROM struct_ceh'
    # sql_str2 = 'SELECT id FROM struct_ceh WHERE caption = ? AND org_id = ?'
    db_conn_str = r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};' \
                  r'DBQ=C:\Users\buhu_\PycharmProjects\OPR\DBksu\ARMv51.MDB;'
    #
    # name, org_id = 'АУ', 2

    con_db = import_json.InterfaceDB(db_conn_str)
    # print(con_db.select_one_from_DB(sql_str))
    # print(con_db.select_one_from_DB(sql_str2, name, org_id))
