...
class InterfaceDB:
    ...
    @staticmethod
    def get_mguid(name_id: int, m_order=1) -> str:
        unique_num = int((datetime.datetime.now().timestamp() % 10) * 1000000000000000)
        return hex(unique_num * m_order + name_id)[2:].upper()
    ...
    def insert_ceh(self, id_org):
        # Перебираем цеха:
        for ceh in self.org_struct['ceh'].values():
            # Создаём кортеж для вставки и вставляем его в БД:
            ceh_tuple = id_org, ceh, 0, self.max_ceh, self.get_mguid(id(ceh), self.max_ceh)
    ...
    def insert_uch(self, uchs, id_ceh, id_par=0):
        ...
        for uch in uchs.values():
            ...
            # Если это участок, вставляем и вызываем вставку дочерних участков:
                ...
                # Создаём кортеж для вставки и вставляем его в БД:
                uch_tuple = id_par, id_ceh, node_level, uch, 0, self.max_uch, self.get_mguid(id(uch), self.max_ceh)
    ...
    def insert_rm(self, rms, id_ceh, id_uch):
        for rm in rms:
            ...
            mguid = self.get_mguid(id(rm), self.max_ceh)
            # Создаём кортеж для вставки и вставляем его в БД:
            kut1 = 2
            ...
            if rm['analog'] > 0:
                ...
                for i in range(rm['analog'] + 1):
                    # Создаём новый уникальный идентификатор:
                    mguid = self.get_mguid(id(i), self.max_ceh)
                    # Создаём кортеж для вставки и вставляем его в БД:
